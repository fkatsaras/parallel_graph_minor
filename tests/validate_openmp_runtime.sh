...
openmp_mul_time=$(run_and_time "openmp_mul" "$MATRIX_A" "$MATRIX_B" "true")
...
if [ "$(echo "$openmp_mul_time < $serial_mul_time" | bc -l)" -eq 1 ]; then
    diff=$(echo "$serial_mul_time - $openmp_mul_time" | bc -l)
    echo "OpenMP implementation is faster than the serial implementation by $diff seconds"
    echo "OpenMP: $openmp_mul_time seconds"
    echo "Serial: $serial_mul_time seconds"
    exit 0
elif [ "$(echo "$openmp_mul_time > $serial_mul_time" | bc -l)" -eq 1 ]; then
    diff=$(echo "$openmp_mul_time - $serial_mul_time" | bc -l)
    echo "Serial implementation is faster than the OpenMP implementation by $diff seconds"
    echo "OpenMP: $openmp_mul_time seconds"
    echo "Serial: $serial_mul_time seconds"
    exit 1
else
